...
class Sprite {
  constructor({
    ...
    // paso parametros con objetos en vez de con muchas propiedades distintas, mas sencillo
...
class Luchador extends Sprite {
  ...
  update() {
    ...
    if (
      this.position.y + this.height + this.velocidad.y >=
      canvas.height - 75
    ) {
      this.velocidad.y = 0;
    } else {
...
var contadorr = 70;
function animate() {
  //esta funcion se esta llamando a si misma, es infinita hasta que acabe el juego (bastantes fps)
  ...
  if (daga.position.x < mago.position.x) {
    ...
    if (
      daga.siendoEmpujado &&
      !mago.habilidadUsada &&
      daga.position.x + daga.width < canvas.width
    ) {
      daga.velocidad.x = 19;
    }
  }
  ...
  if (
    daga.position.y + daga.height + daga.velocidad.y >= canvas.height - 75 &&
  ...
  // Movilidad de MAGO
  if (
    keys.ArrowRight.presionada &&
    mago.UltimaTeclaHorizontal === "ArrowRight" &&
    mago.position.x + mago.width < canvas.width
  ) {
    mago.velocidad.x = 4;
  } else if (
    keys.ArrowLeft.presionada &&
  ...
  // detectar colision
  if (
    daga.ataqueHitbox.position.x + daga.ataqueHitbox.width >= mago.position.x &&
    daga.ataqueHitbox.position.x <= mago.position.x + mago.width &&
    daga.ataqueHitbox.position.y + daga.ataqueHitbox.height >=
      mago.position.y &&
        ...
        setTimeout(function () {
          ...
          daga.imagen =
            daga.position.x < mago.position.x
              ? daga.sprites.quietoInv.imagen
              : daga.sprites.quieto.imagen;
      ...
      if (mago.position.x >= daga.position.x) {
        if (tiempoActual - ultimaVezDisparoFlecha >= 700) {
          ...
          nuevaFlecha.disparar(
            mago.position.x + mago.width,
            mago.position.y + mago.height / 2
          );
